angular.module("saida").controller("saidaCtrl",function($scope, $http){
	$scope.app = "Registro de Saida";
	$scope.materiais = [];
	$scope.modelos =[];
	...
	var carregarMaterial = function () {
	    $http.get("http://localhost:8080/appoficial-0.0.1-SNAPSHOT/saida").then(function (response) {
	...
	$scope.adicionarMaterial = function(material){
		...
		$http.post("http://localhost:8080/appoficial-0.0.1-SNAPSHOT/saida", material).then(function (response) {
	    	delete $scope.material.serie;
	    	delete $scope.material.casid;
	    	delete $scope.material.cartao;
