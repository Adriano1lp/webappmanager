angular.module("adicionaModelo").controller("adicionaModeloCtrl",function($scope, $http){
	...
	$scope.adicionarModelo = function(modelo){
		console.log(modelo);
		$http.post("http://localhost:8080/appoficial-0.0.1-SNAPSHOT/modelo", modelo).then(function (response) {
	...
	$scope.deletarModelo = function (modelos){
		$scope.modelos = modelos.filter(function (modelo){			
			...
			$http.post("http://localhost:8080/appoficial-0.0.1-SNAPSHOT/modelodelete", modelo).then(function (response) {		    	
		    });			
		});        
    };
